...
public class bleWoosimDevice {
     ...
     */
    bleWoosimDevice(Handler handler) {
    ...
    private synchronized void connected(BluetoothSocket socket, BluetoothDevice device, final String socketType) {
        ...
        bleWoosimSdk.mBTConnectResult = 1;
        Message msg = mHandler.obtainMessage(bleWoosimSdk.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(bleWoosimSdk.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        bleWoosimSdk.mBTConnectResult = -3;
        Message msg = mHandler.obtainMessage(bleWoosimSdk.MESSAGE_CONNECTED_FAIL);
        Bundle bundle = new Bundle();
        bundle.putInt(bleWoosimSdk.TOAST, R.string.connect_fail);
        msg.setData(bundle);
        mHandler.sendMessage(msg);

        // Start the service over to restart listening mode
        bleWoosimDevice.this.start();
    }
    ...
    private void connectionLost() {
        ...
        // Send a failure message back to the Activity
        bleWoosimSdk.mBTConnectResult = -2;
        Message msg = mHandler.obtainMessage(bleWoosimSdk.MESSAGE_CONNECTED_FAIL);
        Bundle bundle = new Bundle();
        bundle.putInt(bleWoosimSdk.TOAST, R.string.connect_lost);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        ...
        Setting.setBleName("");
        bleWoosimSdk.getInstance().mConnectResult.onState(false);
...
//                tmp = device.createRfcommSocketToServiceRecord(SPP_UUID);
        ...
        public void run() {
            ...
            synchronized (bleWoosimDevice.this) {
                mConnectThread = null;
            }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    mHandler.obtainMessage(bleWoosimSdk.MESSAGE_READ, bytes, -1, rcvData).sendToTarget();
                } catch (IOException e) {
