...
import static com.jiw.androidpos.sdk.ble.bleDevice.STATE_CONNECTED;
...
import com.jiw.androidpos.sdk.ble.BleRepository;
...
//            return;
//        }
        else if (name.contains(Constants.C1_KMP_102)) {
            registerReceiver(m_ctx);
            mBleRepository.connectDevice(addr, m_ctx, Constants.C1_KMP_102);
...
//                        DisConnect();
    ...
    BroadcastReceiver mBluetoothSearchReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(name != null) {
//                if (!name.contains("KRE") && !name.contains("KMR") && !name.contains("KMP")) {
                if (!name.contains("KRE") && !name.contains("KMP")) {
            ...
            if(Setting.getIsWoosim()) {
                Log.d("Bluetooth id ", " = Woosim or KwangWoo");
                return;
            } else {
                if(name == null) {
...
//            Toast.makeText(m_ctx,"잠시만 기다려 주세요", Toast.LENGTH_LONG).show();
    ...
    public Intent register(BroadcastReceiver receiver, IntentFilter filter) {
        try {
            // ceph3us note:
    ...
    public boolean unregister(BroadcastReceiver receiver, Context _ctx) {
    ...
    private boolean unregisterInternal(BroadcastReceiver receiver, Context _ctx) {
        try {
