...
import static com.jiw.androidpos.sdk.ble.GattAttributes.BLE_NOTIFY;
import static com.jiw.androidpos.sdk.ble.GattAttributes.BLE_RX;
import static com.jiw.androidpos.sdk.ble.GattAttributes.BLE_TRAN;
import static com.jiw.androidpos.sdk.ble.GattAttributes.BLE_TX;
...
import com.jiw.androidpos.sdk.Command;
import com.jiw.androidpos.sdk.Setting;
import com.jiw.androidpos.sdk.Utils;
import com.jiw.androidpos.sdk.van.Constants;
...
public class BluetoothLeService extends Service {
    ...
    private static com.jiw.androidpos.sdk.ble.BleInterface.ResultLinstener mResultLinstener;
    ...
    public final static UUID TRANS_TX = UUID.fromString(com.jiw.androidpos.sdk.ble.GattAttributes.TRANS_TX);
...
//                    e.printStackTrace();
//                }
                if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==40) {
...
//                    return;
                ...
                new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        {
                            if(s.getUuid().equals(com.jiw.androidpos.sdk.ble.GattAttributes.BLE_TRAN))
                            {
                                if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==30) {
...
//                        isSetMtuMode = gatt.requestMtu(com.jiw.testkoces.sdk.ble.bleDevice.Mtu);
                        ...
                        if(Setting.getBleName().contains(Constants.C1_KRE_NEW)) {
                            com.jiw.androidpos.sdk.ble.bleDevice.Mtu = 150;
                            Log.i(TAG, "TryisSetMtuMode = " + com.jiw.androidpos.sdk.ble.bleDevice.Mtu);
                            isSetMtuMode = mBluetoothGatt.requestMtu(com.jiw.androidpos.sdk.ble.bleDevice.Mtu);
                        }
                    }
                }, 500);
    ...
    private void RecvDataUpdate(byte[] recv) {
        ...
        if (mBuffer.length>= com.jiw.androidpos.sdk.ble.GattAttributes.SPEC_MIN_SIZE) {
            if (mBuffer[0] != com.jiw.androidpos.sdk.ble.GattAttributes.STX) /* 만약에 버퍼 시작값이 STX가 아닌 경우 전문 이상으로 보고 모든 버퍼를 지운다.*/ {
                mBuffer = null;
            }
...
//            size[1] = mBuffer[2];
            ...
            if (mBuffer.length >= protlength + com.jiw.androidpos.sdk.ble.GattAttributes.SPEC_HEADER_SIZE)
            {
                if(mBuffer.length == protlength + com.jiw.androidpos.sdk.ble.GattAttributes.SPEC_HEADER_SIZE){
                    mResultLinstener.MessageResultLinstener(mBuffer.clone());
                }else{
                    byte[] temp=new byte[protlength + com.jiw.androidpos.sdk.ble.GattAttributes.SPEC_HEADER_SIZE];
    ...
    private void broadcastUpdate(final String action,
        ...
        */
        ...
        if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==10){
            if(BLE_NOTIFY==null) {
        ...
        else if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==20){
            if(BLE_NOTIFY==null)
            {
                //ble notify uuid 가 설정 되지 않는 경우에 어떻게 처리 할지 결정 해야 한다.
            }
        ...
        else if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==30){
            if(BLE_NOTIFY==null)
            {
                //ble notify uuid 가 설정 되지 않는 경우에 어떻게 처리 할지 결정 해야 한다.
            }
            ...
            if(BLE_RX==null)
            {
                BLE_RX = UUID.fromString(com.jiw.androidpos.sdk.ble.GattAttributes.TRANS_TX);
            }
...
//                }
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==10) {  //payfun 의 경우
...
//                }
                ...
                } else if(com.jiw.androidpos.sdk.ble.GattAttributes.mBleModelType==20) { //아폴로3? 의 경우
                    if (BLE_TX.equals(finalCharacteristic.getUuid())) {
                        BluetoothGattDescriptor descriptor = finalCharacteristic.getDescriptor(com.jiw.androidpos.sdk.ble.GattAttributes.BLE_CHARACTERISTIC_CONFIG);
                        descriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
                        if (descriptor == null) {
