...
public class AES {
    ...
    public byte[] CFB_decrypt(byte[] text) {
        byte[] previousBlock = null;
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        for (int i = 0; i < text.length; i+=16) {
            byte[] part = Arrays.copyOfRange(text, i, i + 16);
    ...
    public byte[] decrypt(byte[] message, byte[] key, String mode, byte[] iv) throws InvalidKeyException, InvalidAlgorithmParameterException {
            ...
            switch (mode) {
                case "CBC":
                    ...
                    if (n != 0) {
                        ...
                        byte temp[] = xor2(cipher.doFinal(ct[ct.length - 2]), ct[ct.length - 1]);
                    ...
                    decrypted[0] = xor2(cipher.doFinal(ct[0]), iv);//First xor with IV parameter
                    for (int i = 1; i < ct.length; i++) {
                        decrypted[i] = xor2(cipher.doFinal(ct[i]), ct[i - 1]);
                    }
...
//                Logger.getLogger(AES.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private byte[] xor2(byte[] array_1, byte[] array_2) {
